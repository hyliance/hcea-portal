...
import { leagueApi, teamsApi, matchFlagApi, GAME_TEAM_SIZES } from '../../api';
...
function LeagueDetail({ league, allTeams, onRefresh, onDelete }) {
  ...
  const handleAdvance = async () => {
    ...
    setSaving(true);
    const res = await leagueApi.advanceStatus(league.id);
    // When league reaches 'complete', stamp eventEndDate on all its flags
    if (res.status === 'complete') {
      await matchFlagApi.setEventEndDate(league.id, Date.now());
    }
    setSaving(false);
    onRefresh();
  };
